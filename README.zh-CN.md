<p align="center">
  <a href="README.md">English</a> |
  <a href="README.zh-CN.md">简体中文</a>
</p>

# Canqihe 的技能集

我的 Claude Code 自定义技能集合，用于提高生产力和自动化。

## 技能列表

| 技能 | 说明 |
|------|------|
| [ai-ui-generator](./ai-ui-generator/) | 通用 AI 驱动的 UI 组件生成器 |
| [algorithmic-art](./algorithmic-art/) | 使用 p5.js 创建算法艺术 |
| [art-master](./art-master/) | 艺术风格提示词生成器 |
| [baoyu-article-illustrator](./baoyu-article-illustrator/) | 分析文章并生成插图 |
| [baoyu-comic](./baoyu-comic/) | 知识漫画创作者 |
| [baoyu-compress-image](./baoyu-compress-image/) | 压缩图像为 WebP 或 PNG 格式 |
| [baoyu-cover-image](./baoyu-cover-image/) | 生成文章封面图片 |
| [baoyu-danger-gemini-web](./baoyu-danger-gemini-web/) | 通过 Gemini Web API 生成图像和文本 |
| [baoyu-danger-x-to-markdown](./baoyu-danger-x-to-markdown/) | 将 X (Twitter) 推文转换为 Markdown |
| [baoyu-format-markdown](./baoyu-format-markdown/) | 格式化纯文本或 Markdown 文件 |
| [baoyu-image-gen](./baoyu-image-gen/) | 使用多个 API 进行 AI 图像生成 |
| [baoyu-infographic](./baoyu-infographic/) | 生成专业信息图 |
| [baoyu-markdown-to-html](./baoyu-markdown-to-html/) | 将 Markdown 转换为样式化的 HTML |
| [baoyu-post-to-wechat](./baoyu-post-to-wechat/) | 发布内容到微信公众号 |
| [baoyu-post-to-x](./baoyu-post-to-x/) | 发布内容到 X (Twitter) |
| [baoyu-slide-deck](./baoyu-slide-deck/) | 生成专业幻灯片图片 |
| [baoyu-translate-local](./baoyu-translate-local/) | 在不同语言之间翻译文章 |
| [baoyu-url-to-markdown](./baoyu-url-to-markdown/) | 将任何 URL 转换为 Markdown |
| [baoyu-xhs-images](./baoyu-xhs-images/) | 小红书信息图系列生成器 |
| [brand-guidelines](./brand-guidelines/) | 应用 Anthropic 品牌颜色和排版 |
| [canvas-design](./canvas-design/) | 创建视觉艺术文档 |
| [design-master](./design-master/) | 平面设计提示词生成器 |
| [doc-coauthoring](./doc-coauthoring/) | 协作文档编写指南 |
| [docx](./docx/) | 全面的文档创建和编辑 |
| [data-analyst](./data-analyst/) | AI 数据分析助手，用于查询 CSV/Excel 文件 |
| [domain-classifier](./domain-classifier/) | AI 领域分类器 |
| [ec-view](./ec-view/) | 电商 KV 视觉系统提示词生成器 |
| [edge-tts-local](./edge-tts-local/) | 使用 Edge TTS 进行文本转语音转换 |
| [find-skills](./find-skills/) | 发现和安装技能 |
| [frontend-design](./frontend-design/) | 创建生产级前端界面 |
| [gemini-watermark-remover](./gemini-watermark-remover/) | 从图像中删除 Gemini AI 水印 |
| [infographic-creator](./infographic-creator/) | 创建精美信息图 |
| [intelligent-prompt-generator](./intelligent-prompt-generator/) | 智能提示词生成器 v2.0 |
| [internal-comms](./internal-comms/) | 撰写内部沟通文档 |
| [ljg-explain-concept](./ljg-explain-concept/) | 深度概念解剖学 |
| [manim-composer](./manim-composer/) | 使用 Manim 创建教育视频 |
| [manimce-best-practices](./manimce-best-practices/) | Manim Community Edition 最佳实践 |
| [manimgl-best-practices](./manimgl-best-practices/) | ManimGL 最佳实践 |
| [mcp-builder](./mcp-builder/) | MCP 服务器创建指南 |
| [morning-report](./morning-report/) | 生成每日早报 |
| [nano-banana-pro](./nano-banana-pro/) | 使用 Google Gemini 3 Pro 生成图像 |
| [notebook](./notebook/) | NotebookLM 多源内容处理器 |
| [notebooklm](./notebooklm/) | 查询 Google NotebookLM 笔记本 |
| [podcast-article-generator](./podcast-article-generator/) | 播客文章生成器 |
| [pdf](./pdf/) | 全面的 PDF 处理工具包 |
| [pptx](./pptx/) | 演示文稿创建和编辑 |
| [port-allocator](./port-allocator/) | 自动分配和管理开发服务器端口 |
| [product-master](./product-master/) | 产品摄影提示词生成器 |
| [prompt-analyzer](./prompt-analyzer/) | 分析和比较提示词 |
| [prompt-extractor](./prompt-extractor/) | 从提示词中提取模块化结构 |
| [prompt-generator](./prompt-generator/) | 从元素数据库生成提示词 |
| [prompt-master](./prompt-master/) | 主提示词控制器 |
| [prompt-xray](./prompt-xray/) | 从提示词中逆向工程知识 |
| [release-skills](./release-skills/) | 技能通用发布工作流 |
| [remotion-best-practices](./remotion-best-practices/) | Remotion 视频创建最佳实践 |
| [seedance](./seedance/) | 为 Seedance 生成视频提示词 |
| [share-skill](./share-skill/) | 自动分享技能并迁移到代码仓库 |
| [skill-creator](./skill-creator/) | 创建有效技能的指南 |
| [skill-i18n](./skill-i18n/) | 将技能文档翻译成多种语言 |
| [skill-permissions](./skill-permissions/) | 分析技能权限并批量授权 |
| [slack-gif-creator](./slack-gif-creator/) | 为 Slack 创建动画 GIF |
| [social-content](./social-content/) | 社交媒体内容创建和优化 |
| [storyboard-generator](./storyboard-generator/) | 为视频生成多镜头分镜 |
| [theme-factory](./theme-factory/) | 使用主题样式化工具包 |
| [ui-check](./ui-check/) | 构建界面的约束条件 |
| [ui-ux-pro-max](./ui-ux-pro-max/) | UI/UX 设计智能 |
| [universal-learner](./universal-learner/) | 从提示词中学习 |
| [video-master](./video-master/) | 视频生成提示词控制器 |
| [video-storyboarding](./video-storyboarding/) | 技术演示视频的前期制作规划 |
| [web-artifacts-builder](./web-artifacts-builder/) | 构建复杂的 HTML artifacts |
| [web-reader-summary](./web-reader-summary/) | Web 阅读器和摘要工具 |
| [web_style](./web_style/) | 网站设计风格生成器 |
| [webapp-testing](./webapp-testing/) | 测试本地 Web 应用 |
| [xhs-images](./xhs-images/) | 小红书图像生成器 |
| [xlsx](./xlsx/) | 全面的电子表格工具包 |
| [youtube-clipper](./youtube-clipper/) | YouTube 视频智能剪辑工具 |
| [z-image](./z-image/) | 使用 ModelScope API 生成图像 |

## 在线文档

本技能集有一个由 [share-skill](./share-skill/) 生成的在线文档网站。

**GitHub Pages：**
```
https://canqihe.github.io/canq-skills/
```

### 设置 GitHub Pages

1. 进入仓库 **Settings** -> **Pages**
2. 在 "Source" 下，选择 **Deploy from a branch**
3. 选择分支：`master`（或 `main`），文件夹：`/docs`
4. （可选）添加自定义域名

## 安装

```bash
# 添加技能市场
/plugin marketplace add canqihe/skills

# 安装单个技能
/plugin install <技能名称>@canqihe-skills
```

## 许可证

MIT

---

Made with ♥ by [Canqihe's skills](https://github.com/canqihe)
