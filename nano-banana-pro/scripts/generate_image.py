...
def load_proxy_config():
    """Load proxy configuration from ~/.gemini_config file."""
    ...
    for line in config_file.read_text().split('\n'):
        ...
        if line and not line.startswith('#') and '=' in line:
            ...
            if key in ('HTTP_PROXY', 'HTTPS_PROXY', 'http_proxy', 'https_proxy'):
                ...
                print(f"Using proxy: {value}")


def get_api_key(provided_key: str | None) -> str | None:
    """Get API key from argument, then config file, then environment."""
    if provided_key:
        return provided_key

    # Try reading from ~/.gemini_api_key file
...
def main():
    # Load proxy configuration before anything else
    load_proxy_config()
    ...
    # Set up output path - default to Desktop if not absolute path
    output_path = Path(args.filename)
    if not output_path.is_absolute():
        desktop_path = Path.home() / "Desktop"
        output_path = desktop_path / args.filename
